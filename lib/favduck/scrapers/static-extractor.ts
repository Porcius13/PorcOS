...
export function extractStaticData(html: string, url: string): ScrapedData | null {
    ...
    try {
        ...
        if (domain.includes('nadirkitap.com')) {
            if (result.price === 0) {
                const nkPrice = $('.nk-sticky-current-price').first().text() || $('p:contains("TL")').first().text();
                if (nkPrice) result.price = smartPriceParse(nkPrice);
            }
            if (!result.title || result.title === '') {
                result.title = $('h1.producTitle').first().text().trim() || $('h1').first().text().trim();
            }
            if (!result.image || result.image === "") {
                const nkImg = $('#urunAnaGorsel').attr('src') || $('.abigimage img').first().attr('src');
                if (nkImg) result.image = nkImg;
            }
        }
