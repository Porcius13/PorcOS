...
export async function scrapeProduct(url: string): Promise<ScrapedData> {
    ...
    return (async () => {
        ...
        const skipStatic = domainName.includes("mavi.com") ||
            ...
            domainName.includes("decathlon.") ||
            domainName.includes("kitapyurdu.com") ||
            domainName.includes("dr.com.tr") ||
            domainName.includes("swatch.com") ||
            domainName.includes("defacto.com") ||
            domainName.includes("nadirkitap.com");
        ...
        if (!skipStatic) {
            ...
            console.log(`[Hybrid Scraper] Skipping static extraction (known JS-heavy site)`);
        }

        // --- STAGE 2: PUPPETEER EXTRACTION (Robust & Guaranteed) ---
        const scraper = getScraper(domainName);

        // For specialized scrapers that handle their own browser (H&M, Mavi, NadirKitap)
        if (domainName.includes("hm.com") || domainName.includes("mavi.com") || domainName.includes("nadirkitap.com")) {
        ...
        try {
            ...
            if (!domainName.includes('nadirkitap.com') && !domainName.includes('decathlon.')) {
            ...
            if (result.price === 0) {
                console.warn(`Zero Price Detected for ${domainName}`, result);
            }
