...
export default function SavingsGoals({ lang }: SavingsGoalsProps) {
    ...
    const fetchGoals = async () => {
        const res = await apiFetch('/api/kasa/goals');
    ...
    const handleContribute = async (id: number, amount: number) => {
        await apiFetch(`/api/kasa/goals-contribute?id=${id}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-black text-foreground tracking-tight text-glow">{t('savingsGoalsTitle', lang)}</h2>
                    <p className="text-sm text-foreground/40 font-medium">{t('savingsGoalsSubtitle', lang)}</p>
                </div>
                <button
                        ...
                        >
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">
                                            {goal.deadline ? `${goal.deadline} ${t('untilDate', lang)}` : t('indefinite', lang)}
                                        </p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-sm font-black text-indigo-600 dark:text-indigo-400">%{Math.round(progress)}</p>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase">{t('completedLabel', lang)}</p>
                                </div>
                            </div>

                            <div className="space-y-2">
                                <div className="flex justify-between text-sm font-bold">
                                    <span className="text-foreground">{new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY' }).format(goal.current_amount)}</span>
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className="flex-1 py-3 bg-foreground/5 rounded-xl text-[10px] font-black uppercase tracking-widest text-foreground/60 hover:bg-foreground/10 transition-all border border-foreground/10"
                                >
                                    +500 ₺ {t('addContribution', lang)}
                                </button>
                                <button
            ...
            <AnimatePresence>
                {isAdding && (
                    <div className="fixed inset-0 z-[100] flex items-end justify-center px-4 pb-4 sm:items-center">
                        ...
                        >
                             <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-6 tracking-tight">{t('newGoalTitle', lang)}</h3>
